...
ggplot() +
  geom_boxplot(data = pl_v02, aes(x = range, y = leaf_area_cm2), 
...
ggplot(pl_v02, aes(x = leaf_area_cm2, y = int_length_cm)) +
  geom_smooth()


# Graph the inflorescence data
ggplot() +
  geom_violin(data = pl_v02, aes(x = range, y = infl_length_cm), 
    width = 0.25, alpha = 0.5) +
